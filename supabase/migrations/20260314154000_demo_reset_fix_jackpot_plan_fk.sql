-- Fix demo reset now that jackpot_payout_plan_steps references jackpot_payout_queue.
-- Reset must clear plan steps before queue truncate.
...
begin
  ...
  -- Child table must be truncated before queue due FK constraint.
  if to_regclass('public.jackpot_payout_plan_steps') is not null then
    execute 'truncate table public.jackpot_payout_plan_steps restart identity';
  end if;
