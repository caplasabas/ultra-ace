...
begin
  ...
  if v_event = 'coins_in' then
    ...
    -- House uses selected profile. Jackpot uses runtime override (or profile template if null).
    -- Split is applied on net spin amount (bet - totalWin), so bucket totals can come down.
    ...
    v_net_split_amount := v_bet - v_spin_win_hint;
    v_house_take := v_net_split_amount * v_house_pct / 100.0;
    v_jackpot_contrib := v_net_split_amount * v_jackpot_pct / 100.0;
    v_pool_contrib := v_net_split_amount * v_happy_pct / 100.0;
