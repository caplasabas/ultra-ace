-- Ensure house/jackpot/happy contributions are computed from net spin result
-- (bet - spin totalWin hint), with priority HOUSE -> JACKPOT -> HAPPY.
...
begin
  ...
  if v_event = 'coins_in' then
    ...
    -- Strict percentage split template:
    --   house% + jackpot% + happy% = 100%
    -- Hierarchy:
    --   1) House is fixed and non-deductible at 20% of BET.
    --   2) Jackpot is allocated next when remaining is positive; when remaining is negative it can go down.
    --   3) Happy gets the final remainder (positive or negative).
    v_house_pct := 20;
    v_jackpot_pct := greatest(coalesce(v_runtime.jackpot_contrib_pct, v_profile_jackpot_pct), 0);
    v_jackpot_pct := least(v_jackpot_pct, greatest(100 - v_house_pct, 0));
    v_happy_pct := greatest(100 - v_house_pct - v_jackpot_pct, 0);

    v_house_take := v_bet * v_house_pct / 100.0;
    v_jackpot_target := v_bet * v_jackpot_pct / 100.0;
    v_after_win := v_bet - v_spin_win_hint;
    v_after_house := v_after_win - v_house_take;

    if v_after_house >= 0 then
      v_jackpot_contrib := least(v_jackpot_target, v_after_house);
      v_pool_contrib := v_after_house - v_jackpot_contrib;
    else
      v_jackpot_contrib := greatest(v_after_house, -v_jackpot_target);
      v_pool_contrib := v_after_house - v_jackpot_contrib;
    end if;
