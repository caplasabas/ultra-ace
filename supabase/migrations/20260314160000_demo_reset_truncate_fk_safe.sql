-- Postgres TRUNCATE + FK rule:
-- parent table cannot be truncated unless child tables are included
-- in the SAME TRUNCATE statement (or CASCADE is used).
-- Update reset function accordingly.
...
begin
  ...
  if to_regclass('public.jackpot_payout_plan_steps') is not null
     and to_regclass('public.jackpot_payout_queue') is not null then
    execute 'truncate table public.jackpot_payout_plan_steps, public.jackpot_payout_queue restart identity';
  elsif to_regclass('public.jackpot_payout_queue') is not null then
    execute 'truncate table public.jackpot_payout_queue restart identity';
  end if;
