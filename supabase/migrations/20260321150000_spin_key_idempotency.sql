create table if not exists public.over_cap_win_events (
  ...
  runtime_mode text not null check (runtime_mode in ('BASE', 'HAPPY')),
  funding_source text not null,
  requested_amount numeric not null check (requested_amount >= 0),
  accepted_amount numeric not null check (accepted_amount >= 0),
  funding_cap_amount numeric not null check (funding_cap_amount >= 0),
  over_amount numeric not null check (over_amount >= 0),
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists idx_over_cap_win_events_event_time
  on public.over_cap_win_events (event_ts desc, id desc);

create index if not exists idx_over_cap_win_events_device_time
  on public.over_cap_win_events (device_id, event_ts desc);
...
grant select on table public.over_cap_win_events to anon;
grant select on table public.over_cap_win_events to authenticated;
grant select on table public.over_cap_win_events to service_role;
grant usage, select on sequence public.over_cap_win_events_id_seq to anon;
grant usage, select on sequence public.over_cap_win_events_id_seq to authenticated;
grant usage, select on sequence public.over_cap_win_events_id_seq to service_role;
grant select on table public.over_cap_win_events_live to anon;
grant select on table public.over_cap_win_events_live to authenticated;
grant select on table public.over_cap_win_events_live to service_role;

create table if not exists public.device_spin_event_dedup (
  id bigint generated by default as identity primary key,
  device_id text not null references public.devices(device_id) on delete cascade,
  spin_key text not null,
  event_type text not null check (event_type in ('bet', 'win', 'spin')),
  event_ts timestamptz not null default now(),
  amount numeric not null check (amount >= 0),
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  unique (device_id, spin_key, event_type)
);

create index if not exists idx_device_spin_event_dedup_device_time
  on public.device_spin_event_dedup (device_id, event_ts desc, id desc);
