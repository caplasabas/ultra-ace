-- Keep both house and jackpot contributions fixed to the configured profile percentages.
-- Only the happy pool absorbs spin win variance.
...
begin
  ...
  if v_event = 'coins_in' then
    ...
    -- House and jackpot always receive their full configured percentages from the bet.
    -- Happy pool gets the residual share and absorbs the spin win.
    v_house_take := greatest(v_house_target, 0);
    v_jackpot_contrib := greatest(v_jackpot_target, 0);
