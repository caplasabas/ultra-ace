-- Jackpot pool + staged payout queue.
-- Accounting priority: HOUSE -> JACKPOT -> HAPPY POOL.

alter table public.devices
  add column if not exists jackpot_contrib_total numeric not null default 0,
  add column if not exists jackpot_win_total numeric not null default 0;

alter table public.device_daily_stats
  add column if not exists jackpot_contrib_amount numeric not null default 0,
  add column if not exists jackpot_win_amount numeric not null default 0;

alter table public.casino_runtime
  add column if not exists jackpot_pool_balance numeric not null default 0,
  add column if not exists jackpot_pool_goal numeric not null default 10000,
  add column if not exists jackpot_contrib_pct numeric not null default 10,
  add column if not exists jackpot_min_winners integer not null default 1,
  add column if not exists jackpot_max_winners integer not null default 5,
  add column if not exists jackpot_delay_min_spins integer not null default 2,
  add column if not exists jackpot_delay_max_spins integer not null default 3,
  add column if not exists jackpot_chunk_min integer not null default 2,
  add column if not exists jackpot_chunk_max integer not null default 3,
  add column if not exists jackpot_win_variance numeric not null default 90,
  add column if not exists jackpot_pending_payout boolean not null default false,
  add column if not exists last_jackpot_triggered_at timestamptz;

create table if not exists public.jackpot_payout_queue (
  id bigserial primary key,
  campaign_id uuid not null default (
    (
  ...
  spins_until_start integer not null default 0,
  payouts_left integer not null default 1,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  completed_at timestamptz
);

create index if not exists idx_jackpot_payout_queue_device_active
  on public.jackpot_payout_queue (device_id, completed_at, created_at);

create index if not exists idx_jackpot_payout_queue_campaign
  on public.jackpot_payout_queue (campaign_id, completed_at);
...
begin
  ...
  if coalesce(v_runtime.jackpot_pool_balance, 0) < greatest(coalesce(v_runtime.jackpot_pool_goal, 0), 0) then
    return jsonb_build_object('triggered', false, 'reason', 'below_goal');
  end if;
  ...
  v_pool := greatest(coalesce(v_runtime.jackpot_pool_balance, 0), 0);
  if v_pool <= 0 then
    return jsonb_build_object('triggered', false, 'reason', 'empty_pool');
  end if;
...
begin
  ...
  if v_event = 'coins_in' then
    ...
    -- Priority split from spin result: HOUSE -> JACKPOT -> HAPPY pool.
    ...
    -- Happy bank still funds HAPPY mode payout accounting.
...
create view public.global_stats_live as
select
  ...
  case
    when coalesce(sum(d.bet_total), 0) > 0
      then round((coalesce(sum(d.win_total), 0) / nullif(sum(d.bet_total), 0)) * 100.0, 4)
    else 0
  ...
  case
    when coalesce(sum(d.bet_total), 0) > 0
      then round((coalesce(sum(d.house_take_total), 0) / nullif(sum(d.bet_total), 0)) * 100.0, 4)
    else 0
