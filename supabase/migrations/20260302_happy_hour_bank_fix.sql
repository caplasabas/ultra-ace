-- Separate accumulating prize pool from active happy-hour payout bank.
-- Prevent flip-flop between BASE/HAPPY when pool oscillates around goal.

alter table public.casino_runtime
  add column if not exists happy_hour_prize_balance numeric not null default 0;
...
begin
  ...
  -- While HAPPY: stay happy until payout bank drains.
  if v_runtime.active_mode = 'HAPPY' then
    if v_runtime.happy_hour_prize_balance <= 0 then
      ...
      v_runtime.active_mode := 'BASE';
    end if;
  else
    -- BASE -> HAPPY transitions:
    -- Manual trigger: move ALL accumulated pool into happy bank.
    if v_runtime.manual_happy_enabled and v_runtime.prize_pool_balance > 0 then
      v_runtime.happy_hour_prize_balance := v_runtime.happy_hour_prize_balance + v_runtime.prize_pool_balance;
      v_runtime.prize_pool_balance := 0;
      v_runtime.active_mode := 'HAPPY';
    -- Auto trigger on goal: move ALL accumulated pool into happy bank.
  ...
  set
    active_mode = v_runtime.active_mode,
...
begin
  ...
  if p_enabled and v_runtime.prize_pool_balance <= 0 and v_runtime.happy_hour_prize_balance <= 0 then
    raise exception 'Cannot enable happy hour: no prize pool available';
  end if;
