-- Device live status + game sessions for dashboard visibility.

alter table public.devices
  add column if not exists current_game_id text,
  add column if not exists current_game_name text,
  add column if not exists device_status text not null default 'idle' check (device_status in ('idle', 'playing', 'offline')),
  add column if not exists active_session_id bigint,
  add column if not exists session_started_at timestamptz,
  add column if not exists session_last_heartbeat timestamptz,
  add column if not exists session_ended_at timestamptz,
  add column if not exists runtime_mode text,
  add column if not exists is_free_game boolean not null default false,
  add column if not exists free_spins_left integer not null default 0,
  add column if not exists pending_free_spins integer not null default 0,
  add column if not exists show_free_spin_intro boolean not null default false,
...
begin
  ...
  -- End any stale active sessions for this device before starting a new one.
