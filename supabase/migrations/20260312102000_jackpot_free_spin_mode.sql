-- Jackpot delivery mode:
-- 1) Winners are still selected by queue with delay.
-- 2) Payout is split into 10 chunks (one per jackpot free spin).
-- 3) Chunks are only paid while device is in free-game mode.
...
-- Migrate existing active single-chunk rows into 10-spin payout mode.
update public.jackpot_payout_queue
set
  payouts_left = 10,
  updated_at = now()
