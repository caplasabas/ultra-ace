-- Profile-driven split with non-negative house/jackpot allocations.
-- Only happy pool can go negative when win exceeds bet.
...
insert into public.rtp_profiles (id, name, mode, house_pct, pool_pct, player_pct, prize_pct, enabled, sort_order)
values
  ('base_slow', 'Base Default 20/20/60', 'BASE', 20, 20, 60, 0, true, 10),
  ('happy_slow', 'Happy Default 20/20/60', 'HAPPY', 20, 20, 60, 0, true, 30),
  ('base_fast', 'Base Fast 10/30/60', 'BASE', 10, 30, 60, 0, true, 20),
  ('happy_fast', 'Happy Fast 20/40/40', 'HAPPY', 20, 40, 40, 0, true, 40)
on conflict (id) do update
set
  name = excluded.name,
  mode = excluded.mode,
  house_pct = excluded.house_pct,
  pool_pct = excluded.pool_pct,
  player_pct = excluded.player_pct,
  prize_pct = excluded.prize_pct,
  enabled = excluded.enabled,
  sort_order = excluded.sort_order,
  updated_at = now();

alter table public.casino_runtime
  alter column jackpot_contrib_pct set default 20;

update public.casino_runtime
set
  base_profile_id = 'base_slow',
  happy_profile_id = 'happy_slow',
  jackpot_contrib_pct = 20,
  updated_at = now()
where id = true;
