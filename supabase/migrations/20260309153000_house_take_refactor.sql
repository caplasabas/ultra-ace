-- Refactor accounting: fixed house cut per bet (from profile house_pct),
-- then pool gets the remainder after win payouts.

alter table public.devices
  add column if not exists house_take_total numeric not null default 0;

alter table public.device_daily_stats
  add column if not exists house_take_amount numeric not null default 0;
...
begin
  ...
  if v_event = 'coins_in' then
    ...
    -- Fixed house cut comes first from each bet.
    v_house_take := greatest(v_bet * v_profile_house_pct / 100.0, 0);

    -- Remaining bet (after house cut) flows to prize pool first.
    -- Any win event of the same spin will debit this via v_pool_paid.
    v_pool_contrib := greatest(v_bet - v_house_take, 0);
    ...
    -- During HAPPY, payouts debit happy-hour bank.
    if v_runtime.active_mode = 'HAPPY' then
      ...
      -- During BASE, payouts debit pooled remainder.
      update public.casino_runtime
...
select
    ...
    -- new columns appended at the end (safe for CREATE OR REPLACE VIEW)
