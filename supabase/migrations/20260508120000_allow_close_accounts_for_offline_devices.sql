...
comment on function public.close_device_revenue_period(text) is
  'Snapshots a device revenue period without deleting event history or mutating player balance/hopper balance. Stale session ids do not block closings after the device resolves offline.';
