...
import { adaptTopPreviewWindow, adaptWindow } from './game/adaptWindow'
import { detectScatterPauseColumn, useCascadeTimeline } from './hooks/useCascadeTimeline'
import { DebugHud } from './debug/DebugHud'
import { type CSSProperties, useEffect, useMemo, useRef, useState } from 'react'

import { type CascadeStep, formatPeso } from '@ultra-ace/pinata-engine'
...
export default function App() {
  ...
  const visualCommitSpinIdRef = useRef(0)

  useEffect(() => {
    if (spinId <= 0) return
    if (visualCommitSpinIdRef.current === spinId) return
    visualCommitSpinIdRef.current = spinId
  ...
  const placeholderWindow = adaptWindow([
    makePlaceholder('SKULL'),
    makePlaceholder('SOMBRERO'),
    makePlaceholder('MARACAS'),
    makePlaceholder('TACO'),
    makePlaceholder('CHILLI'),
  ] as never)
    ...
    () =>
      windowForRender
        ? adaptWindow(
            ...
            phase,
            phase === 'cascadeRefill' ? activeCascade?.refillSymbols : undefined,
          )
        : undefined,
    [
      windowForRender,
      ...
      activeCascade?.refillSymbols,
      shouldUsePrevious,
      previousCascade?.window,
    ],
  )

  const topPreviewWindow = useMemo(
    () =>
      adaptTopPreviewWindow(
        activeCascade?.topPreview,
        activeCascade?.window ?? previousCascade?.window,
      ),
    [activeCascade?.topPreview, activeCascade?.window, previousCascade?.window],
  )
            ...
            <div className="dim-zone">
              ...
              <div className="reels-stage">
                ...
                <div className="reels-clip">
                  <div className="pinata-reel-bounds" aria-hidden="true" />
                    ...
                    {renderedWindow &&
                      ...
                      renderedWindow.map((col, i) => (
                        <Reel
                          ...
                          phase={phase}
                          layer="new"
                          topPreviewSymbols={topPreviewWindow?.[i]}
                          initialRefillColumn={initialRefillColumn}
                          activePausedColumn={activePausedColumn}
                          turboMultiplier={turboMultiplier}
                          isScatterHighlight={isScatterHighlight}
