...
function pickGoldMultiplier(cfg: ReturnType<typeof getEngineConfig>, rng: () => number): number | undefined {
...
export function spin(rng: PRNG, input: SpinInput): SpinOutcome {
  ...
  let freeSpinsAwarded =
    !isFreeGame && scatterCount >= 3
      ? cfg.scatter.baseAward + Math.max(0, scatterCount - 3) * cfg.scatter.extraAwardPerScatter
      : 0

  if (isFreeGame && scatterCount >= 3) {
    freeSpinsAwarded +=
      cfg.scatter.retriggerAward + Math.max(0, scatterCount - 3) * cfg.scatter.extraAwardPerScatter
  }
